...
const Pagination = ({ total, perPage, page, setPage }) => {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6, padding: '12px 0 4px' }}>
      <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
        style={{ padding: '4px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.4 : 1, fontSize: '0.78rem' }}>
        ← Prev
      </button>
      {Array.from({ length: pages }, (_, i) => i + 1).map(n => (
        <button key={n} onClick={() => setPage(n)}
          style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', background: n === page ? 'var(--primary)' : 'var(--surface)', color: n === page ? '#fff' : 'var(--text-primary)', cursor: 'pointer', fontSize: '0.78rem', fontWeight: n === page ? 700 : 400 }}>
          {n}
        </button>
      ))}
      <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages}
        style={{ padding: '4px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', cursor: page === pages ? 'not-allowed' : 'pointer', opacity: page === pages ? 0.4 : 1, fontSize: '0.78rem' }}>
        Next →
      </button>
    </div>
  );
...
export default function CustomerDashboard() {
  ...
  const loadHistory = async (force = false) => {
    ...
    try {
      ...
      setSessions(res.sessions || []); setHistPage(1);
            ...
            <div className="panel-card">
              ...
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {sessions
                      .slice((histPage - 1) * HIST_PER_PAGE, histPage * HIST_PER_PAGE)
                      .map((s, i) => (
                        <tr key={s.id}>
                          <td style={{ color: 'var(--text-muted)' }}>{(histPage - 1) * HIST_PER_PAGE + i + 1}</td>
