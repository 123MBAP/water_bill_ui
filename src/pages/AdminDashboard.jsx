      ...
      <div className="dashboard-page">
        ...
        <Modal isOpen={showUserModal} onClose={closeUserModal} title={editingUser ? 'Edit User' : 'Create New User'}>
          <form className="form-grid" onSubmit={createUser}>
            <label>Full Name<input value={newUser.full_name} onChange={e => setNewUser({ ...newUser, full_name: e.target.value })} required disabled={!!editingUser} placeholder="Full name" /></label>
            <label>Email<input type="email" value={newUser.username} onChange={e => setNewUser({ ...newUser, username: e.target.value })} required disabled={!!editingUser} placeholder="user@example.com" /></label>
            {!editingUser && <label>Password<input type="text" value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} required placeholder="Password" /></label>}
            <label>Role
              <select value={newUser.role} onChange={e => setNewUser({ ...newUser, role: e.target.value })}>
                {roleOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </label>
            {!editingUser && <div className="success-banner" style={{ fontSize: '0.75rem', padding: 'var(--space-sm)', gridColumn: '1 / -1' }}>💡 Default password: <strong>12345678</strong></div>}
            <div className="modal-actions" style={{ gridColumn: '1 / -1' }}>
              <button type="button" className="btn-secondary" onClick={closeUserModal}>Cancel</button>
              <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Saving...' : editingUser ? 'Save Changes' : 'Create User'}</button>
            </div>
          </form>
        </Modal>
