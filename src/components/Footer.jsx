    ...
    <footer className="bg-darknavy text-white">
        ...
        <div className="grid gap-8 sm:gap-10 grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="col-span-2 md:col-span-1 lg:col-span-1">
            ...
            <p className="mt-4 max-w-xs text-sm leading-7 text-slate-400">
              The fastest, most secure way to pay your water bill — anytime, anywhere.
            </p>
            ...
            <div className="mt-6 flex gap-3">
              {['twitter', 'facebook', 'instagram'].map((s) => (
                <a
                  ...
                  href={`#${s}`}
                  aria-label={s}
                  className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-slate-400 transition-all duration-200 hover:bg-primary hover:text-white"
                >
              ...
              <ul className="mt-5 space-y-3">
                {links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-sm text-slate-400 transition-all duration-200 hover:text-white hover:pl-1"
                    >
                      {link.label}
                    </a>
                  </li>
        ...
        <div className="mt-10 sm:mt-14 rounded-3xl bg-white/5 border border-white/10 p-6 sm:p-8 text-center md:text-left md:flex md:items-center md:justify-between gap-6">
          ...
          <form className="mt-4 flex flex-col sm:flex-row gap-2 md:mt-0 md:min-w-[320px]" onSubmit={(e) => e.preventDefault()}>
            <input
              ...
              className="flex-1 min-w-0 rounded-full border border-white/15 bg-white/10 px-5 py-2.5 text-sm text-white placeholder:text-slate-500 outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"
            />
            <button
              type="submit"
      ...
      <div className="border-t border-slate-700/60 bg-[#060f1c]">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-3 px-4 py-5 text-sm text-slate-500 sm:flex-row sm:justify-between sm:px-6 lg:px-8">
          <p>© 2026 AquaPay. All rights reserved.</p>
